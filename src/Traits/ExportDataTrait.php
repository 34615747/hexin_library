...
trait ExportDataTrait
{
    ...
    public function traitRunCommand($query)
    {
        ...
        echo '开始,数量:'.count($ExportJobModels).PHP_EOL;
        foreach ($ExportJobModels as $ExportJobModel){
            ...
            try{
                ...
                $ext = '.csv';
                $file_name = $ExportJobModel->file_name . $ext;
                ...
                $ExportJobModel->file_name = $file_name;
                $ExportJobModel->download_addreee = env('ASYNC_EXCEL_HOST'). '/storage/' . $path;
                ...
                if(CommonHelper::isCli()){
                    echo '已处理:'.$ExportJobModel->id.PHP_EOL;
                }
            }catch (\Exception $e){
                $remark = [
                    'file'=>$e->getFile(),
